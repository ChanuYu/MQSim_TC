...
namespace NVM
{
	namespace FlashMemory
	{
		...
		{
			//function: NVM_PHY_ONFI_NVDDR2::handle_ready_signal_from_chip
		...
		void Flash_Chip::finish_command_execution(Flash_Command* command)
		{
			...
			broadcast_ready_signal(command); //handle_ready_signal_from_chip() 호출
			//실행 이후 호스트쪽으로 보내는 것 처리 (broadcast_transaction_serviced_signal)
		}
		...
		void Flash_Chip::broadcast_ready_signal(Flash_Command* command)
		{
			for (std::vector<ChipReadySignalHandlerType>::iterator it = connectedReadyHandlers.begin();
				...
				(*it)(this, command); //function: NVM_PHY_ONFI_NVDDR2::handle_ready_signal_from_chip
			}
		}
