...
bool Device_Parameter_Set::Ideal_Mapping_Table = true;//If mapping is ideal, then all the mapping entries are found in the DRAM and there is no need to read mapping entries from flash
unsigned int Device_Parameter_Set::Initial_SLC_Blk_Per_Plane = 12; //초기 플레인당 SLC블록 영역 크기
...
void Device_Parameter_Set::XML_serialize(Utils::XmlWriter& xmlwriter)
{
	...
	attr = "Initial_SLC_Blk_Per_Plane";
	val = std::to_string(Initial_SLC_Blk_Per_Plane);
	xmlwriter.Write_attribute_string(attr,val);
...
void Device_Parameter_Set::XML_deserialize(rapidxml::xml_node<> *node)
{
	try
	{
		for (auto param = node->first_node(); param; param = param->next_sibling()) {
			...
			}else if (strcmp(param->name(), "Initial_SLC_Blk_Per_Plane") == 0) {
				std::string val = param->value();
				Initial_SLC_Blk_Per_Plane = std::stoul(val);
