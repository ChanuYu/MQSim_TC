...
namespace SSD_Components
{
	...
	public:
		...
		~Flash_Block_Manager();
		void Allocate_block_and_page_in_plane_for_user_write(const stream_id_type stream_id, NVM::FlashMemory::Physical_Page_Address& address, bool isSLC=false);
		void Allocate_block_and_page_in_plane_for_gc_write(const stream_id_type stream_id, NVM::FlashMemory::Physical_Page_Address& address, bool isSLC=false);
		...
		unsigned int Get_pool_size(const NVM::FlashMemory::Physical_Page_Address& plane_address, bool is_slc = false);
	private:
	};
}
